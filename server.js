// X-Raya (ex-X-Raya) self-hosted сервер (без зависимостей, Node 18+).
...
async function sendEmail(to, subject, html) {
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: process.env.RESEND_FROM || 'X-Raya <onboarding@resend.dev>', to: [to], subject, html }),
  });
...
function handleAuthRequest(req, res) {
  ...
  req.on('end', async () => {
    ...
    try {
      await sendEmail(email, code + ' — код для входа в X-Raya',
        '<div style="font-family:monospace;background:#12100A;color:#EDE6D4;padding:32px;border-radius:12px">' +
        '<div style="color:#F2A93B;font-size:18px;font-weight:bold;margin-bottom:14px">X-RAYA</div>' +
        ...
        '<p><a href="' + link + '" style="display:inline-block;background:#F2A93B;color:#1A1508;padding:12px 22px;border-radius:8px;text-decoration:none;font-weight:bold">Войти в X-Raya</a></p>' +
...
function handleAdminPlan(req, res) {
  ...
  req.on('end', () => {
    ...
    if (plan === 'pro' && !wasPro) {
      ...
      sendEmail(email, 'X-Raya Pro включён',
        '<div style="font-family:monospace;background:#12100A;color:#EDE6D4;padding:32px;border-radius:12px">' +
        '<div style="color:#F2A93B;font-size:18px;font-weight:bold;margin-bottom:14px">X-RAYA</div>' +
    ...
    if (!seenLeads.has(email)) {
      ...
      if (tk && chat) {
        fetch('https://api.telegram.org/bot' + tk + '/sendMessage', {
          ...
          body: JSON.stringify({ chat_id: chat, text: '🎯 X-Raya: новая заявка на Pro\n' + email }),
        }).catch(() => {});
      }
  ...
  req.on('end', () => {
    ...
    if (tk && chat) {
      const text = activated
        ? '🎁 X-Raya: Pro активирован по промокоду ' + promo + '\n' + sess.email + (sess.u.ref ? '\nпартнёр: ' + sess.u.ref : '')
        : '💳 X-Raya: хочет оплатить Pro\n' + sess.email + '\nтариф: ' + tariff +
    ...
    if (activated) {
      const cab = baseUrl(req) + '/cabinet';
      sendEmail(sess.email, 'X-Raya Pro включён',
        '<div style="font-family:monospace;background:#12100A;color:#EDE6D4;padding:32px;border-radius:12px">' +
        '<div style="color:#F2A93B;font-size:18px;font-weight:bold;margin-bottom:14px">X-RAYA</div>' +
...
const server = http.createServer((req, res) => {
  ...
  // X-Raya — бесплатная версия: монетизации нет
  if (req.url === '/api/pro/intent' || req.url === '/api/promo')
    return send(res, 404, 'application/json', JSON.stringify({ error: 'недоступно в этой версии' }));
